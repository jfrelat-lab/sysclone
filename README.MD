...
The ultimate goal of this project is to execute historical and modern programming languages—starting with **MS-DOS QBasic (QB45) and Turbo Pascal (TP7)**—with absolute fidelity, without modifying the original source code, and without freezing the browser's main thread.

## Live Showcases (Running directly in the Browser)
...
[🦍 Gorilla](https://jfrelat-lab.github.io/sysclone/#gorilla) | [🐍 Nibbles](https://jfrelat-lab.github.io/sysclone/#nibbles) | [📊 Sortdemo](https://jfrelat-lab.github.io/sysclone/#sortdemo)
...
* **[Engine Architecture (ARCHITECTURE.MD)](./docs/ARCHITECTURE.MD)**: Deep dive into the Monadic Parsers, Virtual CPU, and Hardware Abstraction Layer.
* **[Project Roadmap (ROADMAP.MD)](./docs/ROADMAP.MD)**: Current progress and what's coming next.
* **[QBasic (QB45) Syntax Reference (QBASIC_REFERENCE.MD)](./docs/QBASIC_REFERENCE.MD)**: A behavioral specification strictly derived from our internal **Truth Vectors**. It defines the formal behavior of every keyword, ensuring the engine's specifications match historical reality.
* **[Turbo Pascal (TP7) Syntax Reference (PASCAL_REFERENCE.MD)](./docs/PASCAL_REFERENCE.MD)**: A behavioral specification strictly derived from our internal **Truth Vectors**, detailing strict memory and typing behaviors.
...
* **Truth Vectors Testing (Single Source Of Truth)**: The absolute source of truth resides in `tests/truth_vectors/*.json`. These vectors define the precise inputs, expected outputs, and hardware side-effects of a given language (e.g., QBasic, Turbo Pascal).
    * **Historical Confrontation**: They serve to confront the historical knowledge of both human and AI architects against real-world behaviors (validated via the DOSBox emulator).
...
* **Auto-Documentation**: The `QBASIC_REFERENCE.MD` and `PASCAL_REFERENCE.MD` are dynamically generated from these vectors, providing an always-accurate, verifiable guide for future developments, and preventing hallucinations.
